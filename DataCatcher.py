...
if __name__ == "__main__":
    ...
    NAME = args.name if args.name is not None else f"DataCatcher.png"
    ...
    hash_params = snapshots["params"]["HashEncoding"]
    hash_net_params = hash_params[:3072]
    hash_grid_params = hash_params[3072:]
    hashnet.load_state_dict({"params": hash_net_params})
    hashgrid.load_state_dict({"params": hash_grid_params})
    ...
    for pixel_index in trange(0, pixels):
        ...
        hash_grid_out = hashgrid(pts_truth)
        hash_features = hashnet(hash_grid_out)
